...
- **Администратори** могат да управляват култури, преработени продукти, дейности по културите, разходи, поръчки и статистики. В мобилното приложение администраторите могат да виждат клиентски поръчки, да ги търсят, да преминават през страници с резултати и да променят статус.
...
- Администраторско управление на поръчки в мобилното приложение: администраторите виждат клиентски поръчки вместо само своите, могат да филтрират по период, потребителско име/имейл и статус, могат да използват странициране и да променят статус от детайлния екран на поръчката.
...
Поведение на API за поръчки:

- Обикновените потребители получават и управляват само собствените си поръчки.
- Администраторите могат да виждат всички клиентски поръчки.
- `GET /api/orders` поддържа query параметри: `page`, `limit`, `status`, `user`, `startDate` и `endDate`.
- `POST /api/orders/:id/edit` позволява на администраторите да променят статуса на всяка поръчка; обикновените потребители могат да променят само своите позволени поръчки.
- Отговорите за поръчки включват `userName` и `userEmail`, които се използват от мобилните админ екрани.
